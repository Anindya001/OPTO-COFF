...
#!/usr/bin/env python3
"""
Optocoupler thermal-cycling reliability study (single-script deliverable).
...
Dependencies allowed by prompt: numpy, scipy, pandas, matplotlib
"""
...
DELTA_T_REFERENCE = 35.0  # near-boundary reference thermal swing in °C
DELTA_T_USE = DELTA_T_REFERENCE  # backward-compatible alias for legacy call sites
CYCLE_TIME_HOURS = 3.0
INPUT_CSV = Path("optocoupler_ttf_unit_level.csv")
USE_EMBEDDED_TOY_DATA = False
...
OUT_DIR = Path("results_optocoupler_reliability")
FIG_DIR = OUT_DIR / "figures"
MAIN_FIG_DIR = FIG_DIR / "main"
SUPP_FIG_DIR = FIG_DIR / "supplementary"
REVISED_FIG_DIR = OUT_DIR / "revised_figures"
...
def build_embedded_toy_dataset(cycle_time_hours: float = 3.0) -> pd.DataFrame:
    """
    Build the unit-level accelerated thermal-cycling dataset.

    Columns:
    unit_id, delta_T, ttf_hours, cycles, event
    """
    stress_data = {
        25.0: [(8200, 1), (9100, 0), (7600, 1), (9400, 0), (6800, 1), (8900, 0), (7300, 1), (9600, 0)],
        50.0: [(4300, 1), (4700, 0), (3900, 1), (4500, 1), (5200, 0), (3600, 1), (4100, 1), (5000, 0)],
        75.0: [(2400, 1), (2100, 1), (2600, 0), (1900, 1), (2300, 1), (2500, 0), (1700, 1), (2000, 1)],
        100.0: [(1500, 0), (1300, 1), (1100, 1), (950, 1), (1200, 1), (1000, 1), (900, 1), (1050, 1)],
        125.0: [(850, 1), (780, 1), (720, 1), (690, 1), (640, 1), (610, 1), (570, 1), (530, 1)],
        150.0: [(620, 1), (560, 1), (500, 1), (470, 1), (430, 1), (390, 1), (360, 1), (330, 1)],
    }

    rows = []
    uid = 1
    for delta_t in sorted(stress_data.keys()):
        unit_list = stress_data[delta_t]
        if len(unit_list) != 8:
            raise ValueError(f"Each stress level must have 8 units; got {len(unit_list)} for stress {delta_t}")
        for cycles, event in unit_list:
            ttf_hours = float(cycles) * cycle_time_hours
            rows.append(
                {
                    "unit_id": f"U{uid:03d}",
                    "delta_T": float(delta_t),
                    "ttf_hours": float(ttf_hours),
                    "cycles": float(cycles),
                    "event": int(event),
                }
            )
    ...
    df = pd.DataFrame(rows, columns=["unit_id", "delta_T", "ttf_hours", "cycles", "event"])
    return df


def load_input_dataset(input_csv: Path, use_embedded_toy_data: bool, cycle_time_hours: float) -> Tuple[pd.DataFrame, str, Optional[Path]]:
    """Load dataset either from CSV (default) or embedded toy dataset (optional)."""
    if use_embedded_toy_data:
        return build_embedded_toy_dataset(cycle_time_hours=cycle_time_hours), "embedded_toy", None

    csv_path = input_csv if input_csv.is_absolute() else (Path(__file__).resolve().parent / input_csv)
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Input CSV not found: {csv_path}. "
            "Set INPUT_CSV to the uploaded real unit-level dataset or set USE_EMBEDDED_TOY_DATA=True for toy data."
        )
...
def print_data_provenance(
    ...
    print("\n================= Data Provenance =================")
    if source_kind == "embedded_toy":
        print("Data source: EMBEDDED TOY DATA (not uploaded real data)")
    else:
        print(f"Data source: CSV file")
        print(f"Input file: {csv_path.resolve() if csv_path is not None else 'N/A'}")
...
def build_censoring_summary_by_stress(data: ReliabilityData) -> pd.DataFrame:
    """Stress-wise censoring burden summary for manuscript reporting."""
    rows = []
...
def run_adaptive_mh_picm_l(
    ...
    for c in range(n_chains):
        ...
        for t in range(total):
            ...
            if (prop[0] <= k_lower) or (prop[1] <= b1_lower) or (not np.isfinite(prop[2])):
                chain_all[t] = theta
                continue
            ...
            # Adapt only in burn-in
            if ((t + 1) % adapt_block == 0) and ((t + 1) <= burnin):
                if len(accepted_states) >= (d + 2):
                    ...
                    prop_cov = (2.38**2 / d) * (emp_cov + 1e-8 * np.eye(d))
                    prop_cov = 0.5 * (prop_cov + prop_cov.T)

        chains[c] = chain_all[burnin:, :]
        accept_rates[c] = accepted / total
        final_covs.append(prop_cov.copy())

    return {"chains": chains, "accept_rates": accept_rates, "final_covs": final_covs}
...
def weibull_quantile(eta: np.ndarray, k: np.ndarray, p: float) -> np.ndarray:
    """Backward-compatible Weibull quantile utility."""
...
def simulate_parametric_bootstrap(data: ReliabilityData, theta: np.ndarray, censor_limits: np.ndarray, rng: np.random.Generator) -> ReliabilityData:
        ...
        rows.append(
            {
                ...
                "name": f"chain_{c}",
                "value": float(acc),
                "warn": bool((acc < 0.15) or (acc > 0.50)),
            }
        )
...
def write_paper_results_summary(
    ...
    """Build and save a concise narrative block for manuscript results text."""
...
def run_prior_sensitivity_picm_l(
    ...
    - B10 at 35C and 25C
    - P(T > mission_cycles) at 35C and 25C
    """
    delta_targets = np.array([35.0, 25.0], dtype=float)
    rows: List[Dict[str, object]] = []
...
def normalize_mojibake_text(text: str) -> str:
    """Repair common UTF-8/Latin-1 mojibake in figure labels before export."""
    if not text or not any(token in text for token in ("Ã", "Â", "Î")):
        return text
    repaired = text
    for _ in range(2):
        if not any(token in repaired for token in ("Ã", "Â", "Î")):
            break
        try:
        ...
        if candidate == repaired:
            break
        repaired = candidate
    return repaired.replace("Â°C", "°C").replace("Î”", "Δ")
...
def save_figure_dual(
    ...
    """Save figure as high-resolution PNG and vector PDF, and register manifest entries."""
    ...
    manifest.append({"filename": str(png_path.relative_to(OUT_DIR)), "category": category, "purpose": purpose})
    manifest.append({"filename": str(pdf_path.relative_to(OUT_DIR)), "category": category, "purpose": purpose})



def _format_delta_t_label(value: float) -> str:
    """Compact ΔT label for figure annotations."""
    return f"ΔT = {int(round(float(value)))} °C"
...
def _draw_reference_markers(
    ...
    labels = {10.0: '10 °C', 15.0: '15 °C', 25.0: '25 °C', 35.0: '35 °C'}
    for val in values:
        ...
        if annotate:
            ax.text(float(val) + 0.6, y_text, labels.get(float(val), f'{float(val):g} °C'), fontsize=7.2, color='#4b5563', va='top')


def _add_regime_bands(ax: plt.Axes, xmin: float, xmax: float) -> None:
    """Background regime shading used in the revised main-paper figures."""
...
def write_figure_manifest(manifest: List[Dict[str, str]], out_path: Path) -> None:
    """Write figure manifest for manuscript assembly."""
...
def plot_main_b10_vs_delta_t(
    ...
    """Main figure: regime-aware B10 comparison across ΔT."""
    ...
    ax.set_xlabel("Thermal excursion ΔT (°C)")
    ...
    save_figure_dual(
        ...
        manifest,
        "Main paper",
        "B10 versus ΔT with PICM-L uncertainty, sensitivity-model medians, mission references, and regime-aware shading for extrapolative, censoring-dominated, and failure-observed domains.",
    )
    plt.close(fig)
...
def plot_main_b10_density_10c_15c(
    ...
    for ax, (idx, d_target) in zip(axes, panels):
        ...
        ax.set_title(f"({panel_tag}) ΔT = {int(d_target)} °C", fontsize=8.5, fontweight="regular")
        style_axis(ax)
    ...
    save_figure_dual(
        ...
        manifest,
        "Supplementary",
        "B10 density comparison at 10 C and 15 C for PICM-L, PICM-C, and lognormal AFT with model-specific median and 95% interval markers.",
    )
    plt.close(fig)
...
def plot_prior_sensitivity_b10_35c(
    ...
    ax.set_xscale("log")
    ax.set_xlabel("B10 life at ΔT = 35 °C (cycles)")
    ...
    save_figure_dual(
        ...
        manifest,
        "Supplementary",
        "PICM-L reference-case B10 density overlay at ΔT = 35 °C for baseline, diffuse, and conservative prior settings.",
    )
    plt.close(fig)
...
def plot_main_failure_fraction_calibration(
    ...
    save_figure_dual(
        ...
        manifest,
        "Main paper",
        "Observed-domain failure-fraction calibration for PICM-L showing posterior-predictive 95% intervals, predictive medians, and observed stress-wise failure fractions.",
    )
    plt.close(fig)
...
def plot_main_loso_observed_vs_pred(
    ...
    for xx, yy, ss, ofail, otot in zip(x, y, stress, chk["observed_failed"].to_numpy(int), chk["observed_total"].to_numpy(int)):
        ax.text(float(xx) + 0.013, float(yy) + 0.012, f"{int(ss)} °C ({ofail}/{otot})", fontsize=8.2, color=PLOT_COLORS["aux"])
    ...
    save_figure_dual(
        ...
        manifest,
        "Supplementary",
        "Supplementary LOSO observed-vs-predicted failure-fraction plot retained for reviewer inspection; main-paper reporting should use the LOSO adequacy table.",
    )
    plt.close(fig)
...
def plot_main_stress_life_logy(
    ...
    save_figure_dual(
        ...
        manifest,
        "Main paper",
        "Stress-life scatter with censoring marks, PICM-L uncertainty, sensitivity-model median curves, and regime-aware shading for extrapolative, censoring-dominated, and failure-observed domains.",
    )
    plt.close(fig)
...
def make_plots(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "main_stress_life.png")
    plt.close(fig)
    ...
    fig.savefig(out_dir / "main_survival_panels.png")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "main_use_b10_posterior.png")
    plt.close(fig)
    ...
    fig.savefig(out_dir / "supp_trace_plots.png")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "supp_marginals_and_prior_posterior.png")
    plt.close(fig)
    ...
    fig.savefig(out_dir / "supp_weibull_probability_plots.png")
    plt.close(fig)
    ...
    ax.legend(h, l, loc="upper right")
    fig.tight_layout()
    fig.savefig(out_dir / "supp_profile_likelihood_b10.png")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "supp_failure_fraction_check.png")
    plt.close(fig)
...
def plot_main_stress_life_comparison(
    ...
    x_use = np.log(delta_t_use)
    ax.axvline(x_use, color=PLOT_COLORS["aux"], lw=1.4, ls=":", label=f"Reference case (ΔT = {delta_t_use:g} °C)")
    ax.text(x_use + 0.01, 0.92 * np.max(y_obs), "Use", fontsize=9, color=PLOT_COLORS["aux"])

    ax.set_xlabel("log(ΔT / °C)")
    ax.set_ylabel("Cycles to failure")
...
def plot_main_reference_case_b10_comparison(
    ...
    ax.text(0.04, 0.12, annotation, transform=ax.transAxes, va="bottom", ha="left", fontsize=7.4, bbox=dict(boxstyle="round,pad=0.20", fc="white", ec="#b5b5b5", alpha=0.94))

    ax.set_xlabel(f"Reference-case B10 at ΔT = {delta_t_use:g} °C (cycles)")
    ax.set_ylabel("Density")
    style_axis(ax)
    h, l = dedup_legend(*ax.get_legend_handles_labels())
    add_figure_legend_above(
    ...
    save_figure_dual(
        ...
        manifest,
        "Main paper",
        "Reference-case B10 posterior at ΔT = 35 °C with Bayesian, MLE, Firth, and profile-likelihood interval comparisons.",
    )
    plt.close(fig)
...
def plot_main_stresswise_life_intervals(
    ...
    ax.set_yticks(ypos)
    ax.set_yticklabels([f"ΔT = {int(s)} °C" for s in stress])
    ax.set_xlabel("Cycles")
...
def plot_supp_leave_one_stress_out(
    ...
    if loso_table is None:
        chk = compute_loso_predictive_adequacy_picm_l(data=data, prior_cfg=get_prior_config(BASELINE_PRIOR_KEY), max_stress=LOSO_MAX_STRESS)
    else:
        chk = loso_table.copy()
    ...
    for xx, yy, ss, rr in zip(x, y, stress, chk.itertuples(index=False)):
        txt = f"{int(ss)} °C ({int(rr.observed_failed)}/{int(rr.observed_total)})"
...
def generate_publication_figures(
    ...
    for target_dir in (main_dir, supp_dir):
    ...
    jobs = [
        ...
        (
            ...
            (post_pred["use_b10_samples"], b10_mle, b10_firth, b10_prof_unc, b10_prof_bc, delta_t_use, main_dir, manifest),
        ),
        (
            "main b10-vs-deltaT",
        ...
        ("supp profile refined", plot_supp_profile_likelihood_refined, (profile_res, bartlett_factor, b10_prof_unc, b10_prof_bc, supp_dir, manifest)),
        ...
        ("supp B10 density 10C-15C", plot_main_b10_density_10c_15c, (pred_picm_l_targets, pred_picm_c_targets, pred_lognorm_targets, supp_dir, manifest)),
    ]
...
def build_table_loso_summary(loso_table: pd.DataFrame) -> pd.DataFrame:
    """Compact LOSO adequacy table intended for manuscript use."""
    if loso_table is None or loso_table.empty:
        return pd.DataFrame(columns=[
            "Held-out ΔT (°C)", "Observed fraction", "Predicted median", "95% PI", "Absolute error", "Coverage"
        ])
    chk = loso_table.sort_values("held_out_stress").reset_index(drop=True).copy()
    return pd.DataFrame({
        "Held-out ΔT (°C)": chk["held_out_stress"].astype(float),
...
def write_loso_summary_outputs(loso_table: pd.DataFrame, out_dir: Path) -> None:
    """Write compact LOSO adequacy outputs for manuscript assembly."""
...
def build_table_d(
    ...
    return pd.DataFrame(
        [
            ...
            {
                "method": "Firth delta-method",
                "delta_T_use": delta_t_use,
                "B10_cycles_estimate": b10_firth[0],
                "lower_95": b10_firth[1],
                "upper_95": b10_firth[2],
            },
            {
...
def export_revised_figure_directory(fig_dir: Path, revised_dir: Path) -> int:
    """Copy current figure directory to a manuscript-facing revised_figure directory."""
    if revised_dir.exists():
        shutil.rmtree(revised_dir)
    shutil.copytree(fig_dir, revised_dir)
    return len(list(revised_dir.rglob("*.*")))


def compare_numeric_tables(old_tables_dir: Path, new_tables_dir: Path) -> pd.DataFrame:
    """Compare selected historical vs revised output tables and summarize numeric deltas."""
    specs = [
        ("table_C_model_comparison.csv", ["model"]),
        ("table_D_use_condition_reliability.csv", ["method"]),
        ("table_low_deltaT_modelwise.csv", ["model", "delta_T"]),
        ("table_low_deltaT_envelope.csv", ["delta_T"]),
        ("table_mission_survival.csv", ["model", "mission_cycles", "delta_T"]),
    ]
    rows: List[Dict[str, object]] = []
    for fname, keys in specs:
        old_path = old_tables_dir / fname
        new_path = new_tables_dir / fname
        if not old_path.exists():
            rows.append({"file": fname, "status": "old_missing", "rows_compared": 0, "numeric_columns": 0, "max_abs_diff": np.nan})
            continue
        if not new_path.exists():
            rows.append({"file": fname, "status": "new_missing", "rows_compared": 0, "numeric_columns": 0, "max_abs_diff": np.nan})
            continue
        try:
            old_df = pd.read_csv(old_path)
            new_df = pd.read_csv(new_path)
        except Exception:
            rows.append({"file": fname, "status": "read_error", "rows_compared": 0, "numeric_columns": 0, "max_abs_diff": np.nan})
            continue

        valid_keys = [k for k in keys if (k in old_df.columns and k in new_df.columns)]
        if not valid_keys:
            rows.append({"file": fname, "status": "no_common_keys", "rows_compared": 0, "numeric_columns": 0, "max_abs_diff": np.nan})
            continue

        merged = old_df.merge(new_df, on=valid_keys, suffixes=("_old", "_new"))
        if merged.empty:
            rows.append({"file": fname, "status": "no_key_overlap", "rows_compared": 0, "numeric_columns": 0, "max_abs_diff": np.nan})
            continue

        numeric_cols_old = [c for c in old_df.columns if c not in valid_keys and pd.api.types.is_numeric_dtype(old_df[c])]
        numeric_cols_new = [c for c in new_df.columns if c not in valid_keys and pd.api.types.is_numeric_dtype(new_df[c])]
        common_numeric = [c for c in numeric_cols_old if c in numeric_cols_new]
        if not common_numeric:
            rows.append(
                {
                    "file": fname,
                    "status": "no_common_numeric_columns",
                    "rows_compared": int(merged.shape[0]),
                    "numeric_columns": 0,
                    "max_abs_diff": np.nan,
                }
            )
        ...
        for c in common_numeric:
            d = np.abs(merged[f"{c}_new"] - merged[f"{c}_old"])
            d = d[np.isfinite(d)]
            if d.size > 0:
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "rows_compared": int(merged.shape[0]),
                "numeric_columns": int(len(common_numeric)),
...
def build_manuscript_values_payload(
    delta_t_use: float,
    ...
    """Build a compact machine-readable manuscript insertion payload."""
...
def write_manuscript_values_files(payload: Dict[str, object], txt_path: Path, json_path: Path) -> None:
    """Write manuscript insertion values in human-readable and machine-readable formats."""
...
def write_revision_log(
    ...
    """Write revision log with changed items, unchanged items, assumptions, and numeric differences."""
    lines = [
        "Revision Log",
        "============",
        "",
        "What changed:",
        "- Prior handling for Weibull shape k is now explicitly Gamma truncated to k > 1.",
        "- Prior hyperparameters are centralized in PRIOR_SETTINGS and exported as tables.",
        "- Model-comparison table now includes 'Uncertainty method' and labels WAIC/DIC as PICM-L only.",
        "- Added LOSO predictive adequacy table and observed-vs-predicted validation figure with uncertainty bars.",
        "- Added prior sensitivity workflow (baseline, diffuse, conservative) and B10@35C overlay figure.",
        "- Added parameterization consistency checks and exported results.",
        "- Added censoring summary table with fully-censored group flag.",
        "- Updated main/supplementary visualization details per manuscript requirements.",
        "",
        "What was not changed:",
        "- Core right-censored likelihood formulations were preserved.",
        "- Main model families (PICM-L Weibull, PICM-C Weibull sensitivity, lognormal AFT sensitivity) were preserved.",
        "- Conservative uncertainty treatment and extrapolation caution were preserved.",
        "",
        "Assumptions:",
    ]
    for a in assumptions:
        lines.append(f"- {a}")
    lines.extend(["", "Numerical differences vs previous outputs:"])
    if numeric_diff_df.empty:
        lines.append("- No comparable historical tables found.")
    else:
        for _, r in numeric_diff_df.iterrows():
            lines.append(
                f"- {r['file']}: status={r['status']}, rows_compared={int(r['rows_compared'])}, "
                f"numeric_columns={int(r['numeric_columns'])}, max_abs_diff={r['max_abs_diff']}"
            )
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_revision_markdown_report(
    out_path: Path,
    numeric_diff_df: pd.DataFrame,
    unresolved_limitations: Sequence[str],
) -> None:
    """Write concise markdown report for code/data revision summary."""
    lines = [
        "# Revision Report",
        "",
        "## What Changed",
        "- Explicit truncated-Gamma prior handling for Weibull shape `k > 1` with effective prior summary export.",
        "- Centralized prior hyperparameters and exported machine-readable prior tables.",
        "- Clarified model uncertainty method in comparison outputs.",
        "- Corrected model-criteria presentation so WAIC/DIC are PICM-L-only and not cross-model implied.",
        "- Added LOSO predictive adequacy workflow and publication-quality validation figure.",
        "- Added PICM-L prior sensitivity workflow and B10@35C prior overlay figure.",
        "- Added parameterization consistency checks and right-censoring summary output.",
        "",
        "## Numerical Impact",
    ]
    if numeric_diff_df.empty:
        lines.append("- Historical comparison tables were unavailable, so numerical delta quantification was limited.")
    else:
        for _, r in numeric_diff_df.iterrows():
            lines.append(
                f"- `{r['file']}`: status={r['status']}, rows={int(r['rows_compared'])}, "
                f"numeric_cols={int(r['numeric_columns'])}, max_abs_diff={r['max_abs_diff']}"
            )
...
def write_run_summary(
    ...
    lines = [
        ...
        f"input_file: {str(csv_path.resolve()) if csv_path is not None else 'EMBEDDED_TOY_DATA'}",
...
def print_summary(
    ...
    print(f"  Firth : k={firth['theta'][0]:.4f}, beta1={firth['theta'][1]:.4f}, beta0={firth['theta'][2]:.4f}")
    print("  Bayes :")
    ...
    if np.any((accept_rates < 0.15) | (accept_rates > 0.50)):
        print("  WARNING: At least one chain acceptance rate is outside [0.15, 0.50].")
    if np.any(rhat > 1.01):
    ...
    for _, r in table_d.iterrows():
        print(
            ...
            f"95% interval=({r['lower_95']:.3f}, {r['upper_95']:.3f})"
        )
    print("==========================================================================\n")
...
def main() -> None:
    ...
    df, source_kind, csv_path = load_input_dataset(
        input_csv=INPUT_CSV,
        use_embedded_toy_data=USE_EMBEDDED_TOY_DATA,
    ...
    if np.any((accept_rates < 0.15) | (accept_rates > 0.50)):
        warnings.warn("At least one chain acceptance rate is outside [0.15, 0.50].")
    if np.any(rhat > 1.01):
    ...
    # 9) Profile likelihood and Bartlett correction for the reference-case B10 (ΔT = 35 °C)
    ...
    # 12) Use-condition B10 summaries for table D (legacy-compatible)
    ...
    narrative = write_paper_results_summary(
        out_file=OUT_DIR / "paper_results_summary.txt",
    ...
    manuscript_payload = build_manuscript_values_payload(
        delta_t_use=DELTA_T_USE,
        waic_dic=waic_dic,
        table_c=table_c,
        table_d=table_d,
    ...
    write_manuscript_values_files(
        payload=manuscript_payload,
        txt_path=OUT_DIR / "manuscript_values.txt",
        json_path=OUT_DIR / "manuscript_values.json",
    )
    (OUT_DIR / "results_summary.json").write_text(json.dumps(manuscript_payload, indent=2, ensure_ascii=True), encoding="utf-8")

    old_tables_dir = Path(__file__).resolve().parent / "old results" / "tables"
    numeric_diff_df = compare_numeric_tables(old_tables_dir=old_tables_dir, new_tables_dir=TAB_DIR)
    numeric_diff_df.to_csv(TAB_DIR / "numeric_differences_vs_old.csv", index=False)
    write_revision_log(
        out_path=OUT_DIR / "revision_log.txt",
        numeric_diff_df=numeric_diff_df,
        assumptions=[
            "LOSO validation uses PICM-L train-only MCMC posterior predictive intervals at held-out runout.",
            "Prior sensitivity survival metrics are reported at DeltaT={35,25} for mission cycles in MISSION_CYCLE_TARGETS.",
            "Cross-model predictive criteria are reported via AIC/BIC; WAIC/DIC remain PICM-L-only under current implementation.",
        ],
    )
    write_revision_markdown_report(
        out_path=OUT_DIR / "revision_report.md",
        numeric_diff_df=numeric_diff_df,
        unresolved_limitations=[
            "WAIC remains unavailable for PICM-C and lognormal in the current implementation.",
            "LOSO for sensitivity models was not run to contain runtime; primary validation focuses on PICM-L.",
        ],
    )

    n_revised_fig = export_revised_figure_directory(FIG_DIR, REVISED_FIG_DIR)
    print(f"Revised figure files copied: {n_revised_fig} -> {REVISED_FIG_DIR.resolve()}")
